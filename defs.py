    ...
    *,
    ...
    permutation_scores = Parallel(n_jobs=n_jobs, verbose=verbose)(
        delayed(_permutation_test_score)(
            clone(estimator),
            X,
            _shuffle(y, None if cv.__class__ == LeaveOneGroupOut else groups, random_state),
            groups,
            cv,
            scorer,
...
def _permutation_test_score(estimator, X, y, groups, cv, scorer, fit_params):
    ...
    for train, test in cv.split(X, y, groups) if cv.__class__ == LeaveOneGroupOut else cv.split(X, y):
...
def permtest(data,PCAdata,indices=None,n_comp=4,subject_level=False, k=5, subset=False,version=None,debug=False):
    """
    ...
    if version == 'PCA':
        Tasklabels,Taskindices, X, y = averageData(data, PCAresults=None, average=False)
    else:
        Tasklabels,Taskindices, y, X = averageData(data, PCAresults=None, average=False)
    ...
    if subset == False:
        for PCnum in range(y.shape[1] if version == 'PCA' else n_comp):
            ...
            kf = LeaveOneGroupOut()
            #kf = KFold(n_splits=k)
            gprmodel = gpr(subject_level=subject_level,refdata=data,version=version,PCnum=PCnum)
            score_gradfac, perm_scores_gradfac, pvalue_gradfac = permutation_test_score_(
                gprmodel,
                X,
                y,
...
class gpr(BaseEstimator):
    ...
    def get_params(self, deep=True):
        return copy.deepcopy({"kernel": self.kernel, "subject_level": self.subject_level, "n_comp": self.n_comp, "refdata": self.refdata,"version":self.version,"PCnum":self.PCnum}) if deep else {"kernel": self.kernel, "subject_level": self.subject_level, "n_comp": self.n_comp, "refdata": self.refdata,"version":self.version,"PCnum":self.PCnum}
    def fit(self, X: np.ndarray, y: np.ndarray,train=None) -> object:
        ...
        if self.version == "PCA":
                ...
                __,_, X, y = averageData(refdata, PCAresults, average= not self.subject_level)
                if self.PCnum != None:
                    y = y [:,self.PCnum]
        else:
            #if self.PCnum != None:
            self.loadings, PCAresults, self.PCModel = PCAfunc(y, self.n_comp,verbose=0)
            __,_, y, X = averageData(refdata, PCAresults, average= not self.subject_level)
            if self.PCnum != None:
                y = y [:,self.PCnum]
...
def PCAfunc(PCAdata: pd.DataFrame, n_comp: int,verbose=1) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def averageData(
    ...
    """
    if isinstance(PCAresults,(np.ndarray,pd.DataFrame,)):
   
        
        FAC = np.asarray(PCAresults).T
        
    else:
        FAC = np.asarray(data.drop(
            [
                "Participant #",
                "Runtime_mod",
                "Task_name",
...
def corr(ref_img, src_img, masker=None, parcel_num=None, parcellation=None,debug=False):
    """
...
def permutation_pipeline(
    ...
    if version=="PCA":
        ...
        scores = permtest(data,PCAdata,subject_level=subject_level, subset=subset,version=version,debug=debug)
...
def func_perm(
    ...
    if version == "GRAD":
        ...
        if subset:
            ...
            permscores = {x:score_permuation["Gradients->PCA"][x][1] for x in score_permuation["Gradients->PCA"]}
            try:
                GradtoPCAscores[parcelnames[parcelval].decode("utf-8")] = [
                    permscores,
                    parcelnames[parcelval].decode("utf-8").split("_")[3],
        ...
        if subset:
            ...
            permscores = {x:score_permuation["PCA->Gradients"][x][1] for x in score_permuation["PCA->Gradients"]}
            try:
                PCAtoGradscores[parcelval] = [
                    permscores,
                    parcelval.split("_")[0],
...
def parcel_dropout(
    data, gradientMaps, taskMaps, parcellation, parcelnames, metric="p-val", subject_level=False,nproc=1,subset=True,debug=False,verbose=1
):
    global debugFlag
    debugFlag = debug
    ...
    if nproc != 1:
        ...
        Parallel(n_jobs=nproc)(delayed(func_perm)(
            data,
            gradientMaps,
            taskMaps,
            i,
